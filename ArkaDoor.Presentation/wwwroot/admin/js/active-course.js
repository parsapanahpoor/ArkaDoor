...
function ActiveCourseSessionFormDone(response) {
    ...
    if (response.status === "Success") {
        ...
        $("#ActiveCoursePendingSessions").load(location.href + " #ActiveCoursePendingSessions");
    }
    else if (response.status === "Error") {
        ShowMessage("اعلان", response.data, "error");
        $("#NormalModal").modal("hide");
...
//#region Pending Sessions Files

function ShowPendingSessionFileManagementModal(sessionId) {
    $.ajax({
        url: "/Admin/ActiveCourse/PendingSessionFilesManagement",
        type: "post",
        data: {
            sessionId: sessionId
        },
        ...
        success: function (response) {
            close_waiting();
            var modalTitle = "مدیریت جلسات در حال بررسی";
            $("#NormalModalTitle").html(modalTitle);
            $("#NormalModalBody").html(response);
            $("#NormalModal").modal("show");
        },
...
//#region Change Session State 

function ChangeSessionStateToRejected(url, sessionId, activeCourseId) {
    $.ajax({
        url: url,//"/Admin/ActiveCourse/ChangeSessionStateToRejected",
        type: "get",
        //data: {
        //    sessionId: sessionId,
        //    activeCourseId: activeCourseId
        ...
        success: function (response) {
            close_waiting();
            var modalTitle = "رد کردن جلسه";
            $("#NormalModalTitle").html(modalTitle);
            $("#NormalModalBody").html(response);
            $('#ChangeSessionStateToRejectedForm').data('validator', null);
            $.validator.unobtrusive.parse('#ChangeSessionStateToRejectedForm');
            $("#NormalModal").modal("show");
        },
        error: function () {
            close_waiting();
...
function ChangeSessionStateToAccepted(url, sessionId) {
    Swal.fire({
        title: 'اعلان',
        text: "آیا از انجام عملیات مورد نظر اطمینان دارید ؟",
        icon: 'warning',
...
function ChangePendingSessionStateToRejected(url, sessionId, activeCourseId) {
    $.ajax({
        url: url,//"/Admin/ActiveCourse/ChangeSessionStateToRejected",
        type: "get",
        //data: {
...
function ChangeSessionStateToRejectedFormDone(response) {
    close_waiting();
    if (response.status === "Success") {
        ShowMessage("اعلان", response.data, "success");
        $("#NormalModal").modal("hide");
...
function ChangePendingSessionStateToAccepted(url, sessionId, activeCourseId) {
    Swal.fire({
        title: 'اعلان',
        text: "آیا از انجام عملیات مورد نظر اطمینان دارید ؟",
        icon: 'warning',
...
function OpenEditTransactionDateModal(id) {
    $.ajax({
        url: "/Admin/ActiveCourse/LoadEditTransactionDateModal",
        type: "post",
        data: {
            registerWalletId: id
        },
        ...
        success: function (response) {
            close_waiting();
            var modalTitle = "ویرایش تاریخ تراکنش";
            $("#SmallModalTitle").html(modalTitle);
            $("#SmallModalBody").html(response);
            $('#EditTransactionDateFrom').data('validator', null);
            $.validator.unobtrusive.parse('#EditTransactionDateFrom');
            var adminDatePickers = $("[AdminDatePicker]");
            if (adminDatePickers.length > 0) {
                $('<link/>',
                    {
...
function EditTransactionDateFromDone(response) {
    close_waiting();
    if (response.status === "Success") {
        ...
        ShowMessage("اعلان", "عملیات با خطا مواجه شد .", "error");
        $("#SmallModal").modal("hide");
    }
}
...
$("#DeleteAttachmentFile").on("click", function () {
...
$("#DeleteMasterVideo").on("click", function () {
    var url = $(this).attr("data-url");
    Swal.fire({
        title: 'اعلان',
        text: "آیا از انجام عملیات مورد نظر اطمینان دارید ؟",
...
//#region Export Registered Student Excel

function LoadRegisteredStudentModalBody(CourseId) {
    $.ajax({
        url: "/Show-RegisteredStudents",
        type: "get",
        data: {
...
//#region Master Wallet

function ShowMasterWalletModal(activeCourseId) {

    debugger;

    $.ajax({
        url: "/Admin/ActiveCourse/LoadMasterWalletModal",
        type: "post",
        data: {
            activeCourseId: activeCourseId
        },
        ...
        success: function (response) {
            ...
            close_waiting();
            var modalTitle = "تسویه حساب مدرس";
            $("#NormalModalTitle").html(modalTitle);
            $("#NormalModalBody").html(response);
            $('#MasterWalletForm').data('validator', null);
            $.validator.unobtrusive.parse('#MasterWalletForm');
            $("#NormalModal").modal("show");
...
function MasterWalletFormDone(response) {
...
function CalculateMasterPercentage() {

    debugger;

    var TotalPayed = parseInt($("#TotalPayed").val());

    var TotalDiscount = parseInt($("#TotalDiscount").val());

    var MasterPercentage = parseInt($("#MasterPercentage").val());

    if (!isNaN(MasterPercentage) && MasterPercentage >= 1 && MasterPercentage <= 100) {

        var price = (TotalPayed /*- TotalDiscount*/) * MasterPercentage / 100;

        var priceFormat = new Intl.NumberFormat().format(price);

        var txtPrice = ` ${priceFormat} تومان`;

        $("#inputMasterWalletPrice").val(txtPrice);
